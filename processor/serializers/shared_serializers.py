...
from processor.models import PriorityOrders,ExemptRule,ThresholdCondition,ExemptConfig,ThresholdAmount,PayPeriod, GarnishmentState, GarnishmentType
...
class ExemptRuleSerializer(serializers.ModelSerializer):
    state = serializers.SlugRelatedField(
        slug_field="state", 
        queryset=GarnishmentState.objects.all()
    )

    class Meta:
        model = ExemptRule
        fields = ["id", "state", "deduction_basis", "withholding_limit", "rule", "created_at", "updated_at"]
    ...
    def update(self, instance, validated_data):
        ...
        if state:
            instance.state = state  # already a GarnishmentState instance

        instance.deduction_basis = validated_data.get("deduction_basis", instance.deduction_basis)
        instance.withholding_limit = validated_data.get("withholding_limit", instance.withholding_limit)
