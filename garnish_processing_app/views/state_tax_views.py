...
from garnish_processing_app.models.garnishment_fees_models import garnishment_fees_states_rule
from garnish_processing_app.models import (garnishment_fees_states_rule,
                      state_tax_levy_config, state_tax_levy_rule_edit_permission, state_tax_levy_exempt_amt_config, state_tax_levy_applied_rule)
from django.core.exceptions import ValidationError
from garnish_processing_app.garnishment_library.utility_class import ResponseHelper
import logging
from garnish_processing_app.garnishment_library.helper import StateAbbreviations
from garnish_processing_app.serializers import (StateTaxLevyRulesSerializers, StateTaxLevyConfigSerializers, StateTaxLevyExemptAmtConfigSerializers, StateTaxLevyRuleEditPermissionSerializers)
from garnish_processing_app.serializers.garnishment_fees_serializers import GarnishmentFeesStatesRuleSerializer
...
class StateTaxLevyConfigAPIView(APIView):
    ...
    def get(self, request, state=None):
        ...
        try:
            if state:
                ...
                rules = state_tax_levy_config.objects.all()
    ...
    def put(self, request, state=None):
        ...
        try:
            ...
            rule = state_tax_levy_config.objects.get(state__iexact=state)
        except state_tax_levy_config.DoesNotExist:
    ...
    def delete(self, request, state=None):
        ...
        if not state:
            return ResponseHelper.error_response('State is required in URL to delete data', status_code=status.HTTP_400_BAD_REQUEST)
        try:
            state_tax_rule = state_tax_levy_config.objects.get(
                state__iexact=state)
