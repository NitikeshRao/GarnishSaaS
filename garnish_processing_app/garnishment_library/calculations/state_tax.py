import logging
from garnish_processing_app.models.state_tax_models import state_tax_levy_config
from garnish_processing_app.models.shared_models import ThresholdAmount
...
from ..utility_class import UtilityClass, CalculationResponse
from garnish_processing_app.serializers.shared_serializers import ThresholdAmountSerializer
...
class StateTaxViewHelper:
    ...
    def get_wl_percent(self, state):
        ...
        try:
            state = state.strip()
            obj = state_tax_levy_config.objects.filter(state__iexact=state)
            if obj.exists():
                return {
    ...
    def get_deduct_from(self, state):
        ...
        try:
            state = state.title()
            obj = state_tax_levy_config.objects.filter(state=state)
            if obj.exists():
...
class StateTaxLevyCalculator(StateWiseStateTaxLevyFormulas):
    ...
    def calculate(self, record, config_data):
        try:
            ...
            disposable_earning = ChildSupportHelper(record.get(EmployeeFields.WORK_STATE)).calculate_de(record)
