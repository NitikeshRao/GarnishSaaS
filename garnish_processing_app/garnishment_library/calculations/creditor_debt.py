from garnish_processing_app.serializers.creditor_debt_serializers import CreditorDebtExemptAmtConfigSerializers
from garnish_processing_app.models.creditor_debt_models import creditor_debt_exempt_amt_config
from rest_framework.response import Response
from rest_framework import status
from .child_support import *
from garnish_processing_app.garnishment_library.helper import *
from garnish_processing_app.garnishment_library.utility_class import UtilityClass, CalculationResponse as CR
...
class CreditorDebtHelper():
    ...
    def _exempt_amt_config_data(self,config_data, state, pay_period,garn_start_date, is_consumer_debt=None, non_consumer_debt=None):
            """
...
class CreditorDebtCalculator(StateWiseCreditorDebtFormulas):
    ...
    def calculate(self, record, config_data):
        ...
        no_of_exemption_including_self = record.get(
            EE.NO_OF_EXEMPTION_INCLUDING_SELF)
        state = StateAbbreviations(record.get(
            EE.WORK_STATE)).get_state_name_and_abbr()
        no_of_dependent_child = record.get(
            EE.NO_OF_DEPENDENT_CHILD)
        filing_status = record.get(EE.FILING_STATUS).lower()
